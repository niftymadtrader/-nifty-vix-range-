...
// ─── SSE: Push to all connected browsers (throttled to 60s) ───
...
function broadcastToClients() {
    ...
    // If 60 seconds have passed since last broadcast, send immediately
    if (now - lastBroadcastTime >= BROADCAST_INTERVAL) {
        doActualBroadcast();
    } else if (!broadcastTimer) {
        // Schedule a broadcast for when the interval completes
    ...
    // Otherwise, a broadcast is already scheduled — just update liveData silently
}
...
function doActualBroadcast() {
    ...
    console.log(`📡 Broadcast sent at ${new Date().toLocaleTimeString('en-IN')} | Clients: ${sseClients.length}`);
}
